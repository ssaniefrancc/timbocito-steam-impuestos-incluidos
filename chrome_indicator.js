...
const getExchangeRate = async () => {
    ...
    let exchangeRateCrypto = JSON.parse(localStorage.getItem('timbocito-cotizacion-crypto'))?.rate;
    let exchangeRateCryptoDate = JSON.parse(localStorage.getItem('timbocito-cotizacion-crypto'))?.rateDateProvided;
    let exchangeRateMep = JSON.parse(localStorage.getItem('timbocito-cotizacion-mep'))?.rate;
    let exchangeRateMepDate = JSON.parse(localStorage.getItem('timbocito-cotizacion-mep'))?.rateDateProvided;
    let tarjetaTax = JSON.parse(localStorage.getItem('timbocito-cotizacion-tarjeta'))?.taxAmount || 10
    let cryptoTax = JSON.parse(localStorage.getItem('timbocito-cotizacion-crypto'))?.taxAmount || 0
...
const renderExchangeIndicator = (exchangeRate,exchangeRateDate,exchangeRateCrypto,exchangeRateCryptoDate,exchangeRateMep,exchangeRateMepDate,tarjetaTax,cryptoTax,mepTax) => {
    ...
    dolarCryptoItem && dolarCryptoItem.addEventListener('click', () => {changePaymentMethodState('timbocito-cotizacion-crypto');window.location.reload()} )

    dolarMepItem && dolarMepItem.addEventListener('click', () => {changePaymentMethodState('timbocito-cotizacion-mep');window.location.reload()} )
}
