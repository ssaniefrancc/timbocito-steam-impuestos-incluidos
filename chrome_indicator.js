...
const getExchangeRate = async () => {
    ...
    let mepTax = JSON.parse(localStorage.getItem('steamcito-cotizacion-mep'))?.taxAmount || 10
...
const renderPriceIndicators = (appData) => {
    return(`
        <p class="reason info">
            Precio sugerido LATAM <br><span class="regional-meter-price">USD$ ${appData.recommendedArsPrice.toFixed(2)}</span>
            ${appData.discount != 0 
                ?
                `<span class="regional-meter-price steamcito-strikethrough-price">USD$ ${appData.baseRecommendedArsPrice}</span>`
                :
                ""
            }
        </p>
        <hr>
        <p class="reason info">
            Precio actual en Paraguay<br><span class="regional-meter-price">USD$ ${appData.arsPrice.toFixed(2)} </span>
            ${appData.discount != 0 
                ?
                `<span class="regional-meter-price steamcito-strikethrough-price">USD$ ${appData.baseArsPrice.toFixed(2)}</span>`
                :
                ""
            }
        </p> 
